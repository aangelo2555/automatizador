...
class SireAjustesHandler {
  ...
  async abrirVentanaAjustes() {
    try {
      ...
      // Cargar la pÃ¡gina de SIRE AJUSTES
      ...
      try {
        ...
        try {
          ...
          throw new Error(`No se encontrÃ³ sire-ajustes.html en ninguna ubicaciÃ³n. Rutas probadas: ${ajustesPath}, ${ajustesPathAlt}`);
        }
      }
  ...
  /**
   * Configura los canales IPC especÃ­ficos para SIRE AJUSTES
   */
  setupAjustesIPC() {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Si aÃºn no existe, devolver datos por defecto
          return {
            ruc: '',
            razonSocial: '',
      ...
      if (!inicioSheet) {
        throw new Error('No se encontrÃ³ la hoja INICIO en AJUSTES.xlsm');
      }
  ...
  async verificarRUC(ruc) {
    try {
      if (!ruc || ruc.length !== 11 || !/^\d+$/.test(ruc)) {
        return { success: false, error: 'Ingrese un RUC vÃ¡lido de 11 dÃ­gitos' };
      }
      ...
      if (response.status === 200 && response.data) {
        ...
        if (razonSocial) {
          ...
          return { success: false, error: 'No se encontrÃ³ RazÃ³n Social' };
        }
      } else {
        return { success: false, error: `Error de conexiÃ³n API. CÃ³digo: ${response.status}` };
      }
    } catch (error) {
      logger.error('Error en verificaciÃ³n de RUC', { error: error.message, ruc });
  ...
  /**
   * Completa datos segÃºn la hoja especificada
   */
  async completarDatos(nombreHoja, datosEmpresa) {
    try {
      ...
      try {
        ...
        logger.info('Archivo AJUSTES.xlsm leÃ­do correctamente');
      } catch (readError) {
      ...
      if (!sheet) {
        ...
        // Buscar una hoja compatible segÃºn el modo
      ...
      // Determinar configuraciÃ³n segÃºn el tipo de hoja
      let config = this.obtenerConfiguracionHoja(hojaUsada);

      if (!config) {
        logger.warn(`No hay configuraciÃ³n especÃ­fica para ${hojaUsada}, usando configuraciÃ³n por defecto`);
        config = { filaInicio: 18, columnaReferencia: 'G' };
      }

      // Buscar Ãºltima fila con datos o crear datos de ejemplo
  ...
  /**
   * Obtiene la configuraciÃ³n especÃ­fica para cada tipo de hoja
   */
  ...
  /**
   * Busca la Ãºltima fila con datos en una columna especÃ­fica
   */
  ...
  /**
   * Crea una hoja de ejemplo con estructura bÃ¡sica
   */
   ...
   * @param {string} ruc - NÃºmero de RUC para buscar en subcarpeta especÃ­fica (opcional)
  ...
  async listarArchivosOutput(ruc = null, rutaBase = null) {
    try {
      // Determinar ruta de bÃºsqueda base
      ...
      try {
        ...
        // Solo intentamos crear si es la ruta por defecto. Si es ruta custom y no existe, devolvemos vacÃ­o.
        if (!rutaBase) {
      ...
      // Si se proporciona RUC, buscar en la subcarpeta especÃ­fica
      if (ruc) {
  ...
  async leerArchivoXlsxOutput(nombreArchivo, ruc = null, rutaBase = null) {
    try {
      ...
      // Determinar la ruta de bÃºsqueda completa
      let rutaArchivo;
      if (ruc) {
      ...
      try {
        ...
        return { success: false, error: `No se encontrÃ³ el archivo: ${nombreArchivo}` };
      }

      // Verificar que es un archivo xlsx
      const extension = path.extname(nombreArchivo).toLowerCase();
      if (extension !== '.xlsx' && extension !== '.xls' && extension !== '.xlsm') {
        return { success: false, error: 'El archivo no es un archivo Excel vÃ¡lido' };
      }
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        const datosJson = XLSX.utils.sheet_to_json(sheet, {
          header: 1, // Usar Ã­ndices numÃ©ricos como headers
          defval: '', // Valor por defecto para celdas vacÃ­as
          raw: false // Convertir valores a strings para mejor visualizaciÃ³n
        });
        ...
        const headersConocidos = ['ruc', 'razon social', 'razÃ³n social', 'periodo', 'car sunat', 'fecha de emision', 'fecha de emisiÃ³n', 'tipo cp/doc', 'serie'];
        ...
        // Obtener datos (filas despuÃ©s de los headers)
      ...
      logger.info('Archivo xlsx leÃ­do correctamente', {
        nombreArchivo,
  ...
  async cambiarVista(modo) {
    try {
      // En una implementaciÃ³n real, esto modificarÃ­a la visibilidad de las hojas
  ...
  /**
   * Genera archivo TXT y ZIP segÃºn la hoja especificada
   */
  async generarArchivo(nombreHoja, rutaDestino) {
    try {
      ...
      try {
        ...
        return { success: false, error: 'No se encontrÃ³ el archivo AJUSTES.xlsm' };
      }
      ...
      if (!sheet) {
        return { success: false, error: `No se encontrÃ³ la hoja ${nombreHoja}` };
      }

      // Obtener configuraciÃ³n de generaciÃ³n
      const configGen = this.obtenerConfiguracionGeneracion(nombreHoja, workbook);
      if (!configGen) {
        return { success: false, error: `No hay configuraciÃ³n de generaciÃ³n para ${nombreHoja}` };
      }

      // Generar nombre de archivo
      const nombreArchivo = this.generarNombreArchivo(nombreHoja, workbook, configGen);

      // MODIFICACIÃ“N: Crear subcarpeta con el RUC
      ...
      return {
        success: true,
        message: `Archivo generado con Ã©xito para: ${nombreHoja}`,
   ...
   * Este es el mÃ©todo principal para generar archivos RVIE y RCE
   * 
   ...
   * @param {Object} datosEmpresa - Datos de la empresa (RUC, razÃ³n social, aÃ±o, mes)
   * @param {string} correlativo - NÂ° Correlativo para el nombre del archivo
  ...
  async generarArchivoDesdeTabla(nombreHoja, rutaDestino, datosTabla, datosEmpresa, correlativo = '01', indicadorCont = '1', indicadorMoned = '1', comprobPago = 'CP') {
    try {
      ...
      if (!rutaDestino) {
        return { success: false, error: 'No se especificÃ³ la carpeta de destino' };
      }
      ...
      // Generar nombre de archivo segÃºn estÃ¡ndares SUNAT (pasando correlativo e indicadores)
      ...
      return {
        success: true,
        message: `Archivos TXT y ZIP generados con Ã©xito`,
        nombreArchivo: nombreArchivo,
        rutaTXT: rutaCompleta,
        rutaZIP: rutaZIP,
  ...
  /**
   * Obtiene la configuraciÃ³n de generaciÃ³n para tablas editables
   */
  obtenerConfiguracionGeneracionTabla(nombreHoja) {
    const configuraciones = {
      // RVIE (Ventas) - Columnas segÃºn estructura de la tabla editable
      'RVIE COMPLEMENTA': {
        tipoNombre: 'cpf_rvie',
      ...
      'RVIE REEMPLAZA': {
        ...
        // 34 columnas totales segÃºn estÃ¡ndar SUNAT
        columnas: ['col1_ruc', 'col2_razonSocial', 'col3_periodo', 'col4_carSunat',
          'col5_fecEmision', 'col6_fecVence', 'col7_tipo', 'col8_serie',
      ...
      // RCE (Compras) - 36 columnas segÃºn estructura SUNAT
      'RCE COMPLETA': {
      ...
      // RCE NO DOMICILIADOS - 36 columnas segÃºn estructura SUNAT
      'RCE NO DOMICILIADOS': {
      ...
      // RCE REEMPLAZA - 50 columnas segÃºn estructura SUNAT
      ...
      // RCE AJUSTES POST 1 - 37 columnas principales + Libre UtilizaciÃ³n (42-80)
      // Formato nombre: LE{RUC}{YYYYMM}00{LLLLLL}{CC}{O}{I}{M}{G}{NN}.txt (35 caracteres)
      // Estructura: Generador, Periodo, CAR-SUNAT, Comprobante, Proveedor, Bases Imponibles, Tributos, Referencia, ClasificaciÃ³n
      'RCE AJUSTES POST 1': {
        ...
        columnas: [
          ...
          // Libre UtilizaciÃ³n (columnas 42-80)
  ...
  /**
   * Genera el nombre del archivo desde datos de la tabla segÃºn estÃ¡ndares SUNAT
   * @param {string} nombreHoja - Nombre de la hoja
   * @param {Object} datosEmpresa - Datos de la empresa
   * @param {Object} config - ConfiguraciÃ³n de generaciÃ³n
   * @param {string} correlativo - NÂ° Correlativo (para RVIE COMPLEMENTA y RCE COMPLETA)
  ...
  generarNombreArchivoDesdeTabla(nombreHoja, datosEmpresa, config, correlativo = '01', indicadorCont = '1', indicadorMoned = '1', comprobPago = 'CP') {
    ...
    switch (config.tipoNombre) {
      ...
      // LLLLLL=080400, CC=03, O=1, I=indCont, M=indMoned, G=2(fijo), NN=correlativo 2 dÃ­gitos
      // Total: 35 caracteres segÃºn estÃ¡ndar SUNAT
  ...
  generarContenidoTXTDesdeTabla(datosTabla, nombreHoja, config) {
    ...
    for (const fila of datosTabla) {
      ...
      // Recorrer las columnas definidas en la configuraciÃ³n
      ...
      // Unir campos con pipe (|) y agregar pipe final segÃºn estÃ¡ndar SUNAT (excepto RCE NO DOMICILIADOS)
      const lineaUnida = campos.join('|');
      lineas.push(sinPipeFinal ? lineaUnida : lineaUnida + '|');
    }

    // Retornar sin lÃ­nea vacÃ­a al final
    return lineas.join('\n');
  }

  /**
   * Obtiene la configuraciÃ³n de generaciÃ³n para cada tipo de hoja segÃºn estÃ¡ndares SUNAT
  ...
  obtenerConfiguracionGeneracion(nombreHoja, workbook) {
    const configuraciones = {
      ...
      // Hojas RVIE (Ventas) - SegÃºn estÃ¡ndares SUNAT
  ...
  /**
   * Genera el nombre del archivo segÃºn el tipo de hoja - EstÃ¡ndares SUNAT
   * 
  ...
  generarNombreArchivo(nombreHoja, workbook, config) {
    ...
    switch (config.tipoNombre) {
      ...
      // ========== HOJAS RVIE (VENTAS) - ESTÃNDARES SUNAT ==========
  ...
  /**
   * Genera el contenido del archivo TXT segÃºn estÃ¡ndares SUNAT
   * Formato: campo1|campo2|campo3|...|campoN|
   * Cada lÃ­nea termina con pipe (|)
   * Sin lÃ­nea vacÃ­a al final
   */
  generarContenidoTXT(sheet, config) {
    const lineas = [];
    const range = XLSX.utils.decode_range(sheet['!ref']);

    // Buscar Ãºltima fila con datos (verificando columna B)
    let ultimaFila = 0;
    ...
    // Generar lÃ­neas del archivo segÃºn estÃ¡ndar SUNAT
    for (let fila = config.filaInicio - 1; fila <= ultimaFila; fila++) {
      const campos = [];

      // Columna de inicio (generalmente C = Ã­ndice 2)
      ...
      for (let col = colInicio; col < config.columnaFin; col++) {
        ...
        if (sheet[cellAddress]) {
          ...
          if (cell.t === 'n') {
            // NÃºmero: formatear sin notaciÃ³n cientÃ­fica
      ...
      // Unir campos con pipe (|) y agregar pipe final segÃºn estÃ¡ndar SUNAT
  ...
  /**
   * Formatea nÃºmeros segÃºn estÃ¡ndar SUNAT (sin notaciÃ³n cientÃ­fica, mÃ¡ximo 2 decimales para montos)
   */
  formatearNumeroSUNAT(valor) {
    ...
    // Si tiene decimales, formatear con mÃ¡ximo 2 decimales
    return num.toFixed(2);
  }

  /**
   * Formatea fechas segÃºn estÃ¡ndar SUNAT (dd/mm/yyyy)
   */
  ...
  /**
   * Carga los datos de una hoja especÃ­fica
   */
  ...
  cargarDatosRealesHoja(sheet, nombreHoja) {
    try {
      ...
      // Leer datos desde la fila de inicio hasta la Ãºltima fila con datos
      for (let row = config.filaInicio - 1; row <= range.e.r; row++) {
        // Verificar si la fila tiene datos (columna B no vacÃ­a)
  ...
  async guardarDatosEditados(nombreHoja, datosEditados) {
    try {
      ...
      // Obtener configuraciÃ³n de la hoja
      ...
      // Determinar quÃ© datos usar (datosUnificados tiene prioridad para tablas RVIE)
      const datos = datosEditados.datosUnificados || datosEditados.datosGenerador || [];

      if (datos.length > 0) {
        // Guardar segÃºn el tipo de hoja
  ...
  /**
   * Guarda datos especÃ­ficos para hojas RVIE (57, 51 o 50 columnas)
   */
  guardarDatosRVIE(sheet, config, datos, nombreHoja) {
    datos.forEach((row, index) => {
      const fila = config.filaInicio + index;

      // Columna B: ID/NÃºmero secuencial
      this.setCellValue(sheet, fila, 1, row.id || index + 1);

      // Columnas segÃºn estructura RVIE (C en adelante = Ã­ndice 2+)
      // Columna C (2): RUC
      this.setCellValue(sheet, fila, 2, row.col1_ruc || '');
      // Columna D (3): RazÃ³n Social
      ...
      // Columna O (14): RazÃ³n Social Cliente
      this.setCellValue(sheet, fila, 14, row.col13_razonSocialCliente || '');
      // Columna P (15): Valor ExportaciÃ³n
      ...
      // Columna U (20): OperaciÃ³n Exonerada
      this.setCellValue(sheet, fila, 20, row.col19_operacionExonerada || '');
      // Columna V (21): OperaciÃ³n Inafecta
      ...
      // Columna AE (30): Fecha EmisiÃ³n Ref
      ...
      // Columna AH (33): NÃºmero Ref
      this.setCellValue(sheet, fila, 33, row.col32_numeroRef || '');
      // Columna AI (34): IdentificaciÃ³n Contrato
      ...
      if (nombreHoja === 'RVIE COMPLEMENTA') {
        ...
        // Columnas de libre utilizaciÃ³n (41-57)
        for (let i = 41; i <= 57; i++) {
          this.setCellValue(sheet, fila, i + 1, row[`col${i}_libre`] || '');
        }
      } else {
        // Para RVIE REEMPLAZA y AJUSTES POST: columnas de libre utilizaciÃ³n
        for (let i = 41; i <= 57; i++) {
  ...
  /**
   * Establece el valor de una celda especÃ­fica
   */
  setCellValue(sheet, fila, columna, valor) {
    ...
    if (valor !== null && valor !== undefined && valor !== '') {
      ...
      // Si el valor estÃ¡ vacÃ­o, eliminar la celda
      delete sheet[cellAddress];
    }
  }
}
